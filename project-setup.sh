...
enableSite() {
	read -p "Do you want to enable the new site? (y/n)" enable
	echo ""
	...
	if ${enable} = "y"
		then
			...
			read -p "Do you want to restart nginx for the changes to take effect? (y/n)" restart
			echo ""
			if ${restart} = "y"
				then
...
read -p "Do you want to setup a new nginx config file for this project? (y/n)" setupNginx
echo ""
